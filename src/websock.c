...
void libwebsock_insert_into_thread_list(libwebsock_client_state *state, pthread_t *thread, enum WS_THREAD_TYPE type){
...
void libwebsock_handshake_finish(struct bufferevent *bev, libwebsock_client_state *state){
	...
	if (state->onopen != NULL){
		...
		pthread_create(onopen_thread, NULL, libwebsock_pthread_onopen, (void *)state);
